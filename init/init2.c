...
/*   Updated: 2023/02/27 03:06:03 by jewancti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_images(t_data *data)
{
	const char	*path_textures[TEXTURE_SIZE] = {
		data -> map.path_no, data -> map.path_so,
	...
	{
		data -> xpm[index] = mlx_xpm_file_to_image(data -> mlx_ptr, \
										(char *)path_textures[index], & w, & h);
		if (!data -> xpm[index])
			return (ft_printf("{blue}init_images: {red}Failed %s{reset}\n", \
						path_textures[index]));
		data -> xpm_addr[index] = (int *)mlx_get_data_addr(data -> xpm[index], \
